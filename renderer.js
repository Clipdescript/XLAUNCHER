        ...
        try {
            ...
            if (modpacks && modpacks.length > 0) {
                ...
                modpacks.forEach(pack => {
                    ...
                    card.innerHTML = `
                        <div style="height: 140px; background: #555; display: flex; align-items: center; justify-content: center; position: relative;">
                            ${iconHtml}
                            <span style="font-size: 40px; position: absolute; z-index: 0;">📦</span>
                        </div>
                        <div style="padding: 15px; flex: 1; display: flex; flex-direction: column;">
                            <h3 style="margin: 0 0 5px 0; font-size: 16px;">${pack.title}</h3>
                            <p style="font-size: 12px; color: #aaa; margin: 0 0 10px 0; flex: 1; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical;">${pack.description}</p>
                            <button class="install-modpack-btn" data-slug="${pack.slug}" data-title="${pack.title}" style="width: 100%; padding: 8px; background: #4CAF50; border: none; color: white; cursor: pointer; border-radius: 3px; margin-top: auto;">INSTALLER</button>
                        </div>
                    `;
